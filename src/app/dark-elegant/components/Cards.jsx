	...
	return (
		<motion.figure variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} className="lg:w-1/3 p-5">
			...
			<figcaption className="my-5 text-center">
				<motion.h3 variants={fadeInUp} className={`${imperial.className} text-5xl text-yellow-200`}>
					{name}
				</motion.h3>
				<motion.div variants={fadeInUp} className="text-white/80 mt-2">
					{desc}
				</motion.div>
			</figcaption>
		</motion.figure>
...
export function RundownCard({ agenda, location, date, time }) {
	return (
		...
		>
			<motion.h3 variants={fadeInUp} className={`${imperial.className} text-6xl text-yellow-100`}>
				{agenda}
			</motion.h3>
...
export function GiftCard({ bank, number, name }) {
	return (
		...
		>
			...
			<motion.h3 variants={fadeInUp} className={`${serif.className} text-2xl font-bold tracking-wide text-yellow-100`}>
				{bank}
			</motion.h3>
